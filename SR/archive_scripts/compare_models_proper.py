#!/usr/bin/env python3
"""
Proper comparison: Create LR from HR, then upscale
"""
...
def create_comparison_visualization(hr_original, lr_input, pretrained, finetuned,
    ...
    axes[0, 0].set_title(f'Ground Truth (HR Original)\n{hr_original.shape[1]}x{hr_original.shape[0]}',
    ...
    zoom_size = min(hr_rgb.shape[0]//2, hr_rgb.shape[1]//2, 40)
    h, w = hr_rgb.shape[:2]
...
def main():
    ...
    results_dir = base_dir / 'results_proper_comparison'
    ...
    # Get cropped images (these are our HR ground truth)
    image_files = []
    ...
    print(f"Found {len(image_files)} HR images to process\n")
    ...
    print("\nProcessing images (creating LR → upscaling → comparing with HR)...\n")
    all_metrics = []
    ...
    for idx, img_path in enumerate(tqdm(image_files, desc="Processing")):
        try:
            ...
            # Create LR by downsampling (simulate low-resolution input)
            h, w = hr_original.shape[:2]
            lr_input = cv2.resize(hr_original, (w//4, h//4), interpolation=cv2.INTER_CUBIC)

            # Add slight blur to simulate real degradation
            ...
            # Calculate metrics (compare with HR ground truth)
    ...
    # Create summary
    print("\n" + "="*80)
    print("PROPER EVALUATION RESULTS (LR → SR → compare with HR)")
    print("="*80)
    ...
    ax1.set_title('Average PSNR (vs Ground Truth)', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('Average SSIM (vs Ground Truth)', fontsize=14, fontweight='bold')
